...
test("architecture layer classification covers the migration buckets", () => {
  ...
  assert.equal(classifyArchitectureLayer("src/notion-cli/page-markdown-adapter.mjs"), "notion-cli-adapter");
  assert.equal(classifyArchitectureLayer("src/validators.mjs"), "validators");
  assert.equal(classifyArchitectureLayer("test/cli/cli.test.mjs"), "tests");
...
test("architecture inventory rejects test dependencies on local-only task artifacts", () => {
  ...
  const result = buildFixtureInventory([
    {
      path: "test/package/local-artifacts.test.mjs",
      source: `
...
test("architecture inventory rejects root-level tests outside layer directories", () => {
  const result = buildFixtureInventory([
    {
      path: "test/flat.test.mjs",
      source: "import test from \"node:test\";",
    },
    {
      path: "test/cli/cli.test.mjs",
      source: "import test from \"node:test\";",
    },
  ]);

  assert.equal(result.ok, false);
  assert.ok(violationCodes(result).includes("architecture.test-not-layered"));
});
